...
module.exports = async (client, { messages, type }) => {
  ...
  if (getCommand) {
    ...
    const command_log = [
      ...
      chalk.blue(msg.pushName),
      chalk.bgGray(msg.senderNumber),
    ];
    if (msg.isGroup) {
    ...
    if (
      ...
      !config.ownerNumber.includes(msg.senderNumber)
    ) {
    ...
    return getCommand
      .callback({ client, message, msg, command, prefix, args, fullArgs })
      .then(async () => {
